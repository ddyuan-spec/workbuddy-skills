 ...
 *
 * 可选尺寸参数（默认 1440×1000）:
 *   --width 1920 --height 920        # 全局；manifest 里也可在组对象上写 "w"/"h" 覆盖
 *   用途：与真实后台参考截图做「同尺寸 1:1 对图」（PROTO-REPLICA-001 复刻校验），
 *         参考图多少像素，就把视口调成多少像素，再用 PIL 上下拼图目视比对
 *   注：sel 传 "body" 即整页截图（含顶栏/侧栏），传 "#viewXxx" 只截该视图容器
 *
 ...
 *   7) 【2026-09-17 实测】同一 file 组内多个 entry 复用同一个 page 顺序执行、状态会累积：
 *      若前一个 entry 开了弹窗，后面所有 entry 的截图都会被弹窗遮住（弹出层是 fixed 全屏）。
 *      修法：后续 entry 的 steps 里先关闭（如 "closeXxx()"），不要新开 page 也不要删截图重跑整轮。
 *      同理：entry 顺序应「先看默认态 → 再看打开态」，避免来回切换残留。
 */
...
function parseArgs(argv) {
  const a = { proto: null, map: null, manifest: null, w: null, h: null };
  for (let i = 0; i < argv.length; i++) {
    if (argv[i] === '--proto') a.proto = argv[++i];
    else if (argv[i] === '--map') a.map = JSON.parse(argv[++i]);
    else if (argv[i] === '--manifest') a.manifest = argv[++i];
    else if (argv[i] === '--width') a.w = Number(argv[++i]);
    else if (argv[i] === '--height') a.h = Number(argv[++i]);
  }
  return a;
}
...
(async () => {
  ...
  for (const g of groups) {
    ...
    const page = await browser.newPage({ viewport: {
      width: g.w || args.w || 1440,
      height: g.h || args.h || 1000
    } });
