...
def scan_page_structure(html_text):
    """扫描 §四 中「本期新增/改动」页面是否齐备 docx 每页模板必备子节（2.1~2.2）。

    规则对应 references/prd-detail-template.md「二、单页标准结构」，即团队
    《PRD 输出规范 260715》示例的每页骨架：
      - 2.1 页面原型与交互示意（必须贴 <img> 原型截图）
      - 2.2 详细逻辑规则（界面与展示规则 + 业务逻辑与边界条件）
    这两项被评审证实是「每页说明被简化」的高发区——只写一段扁平描述、不贴图、
    不写界面/边界规则。本扫描专门兜住这两点。

    返回 (blockers, warns)：
      blockers = [(heading, [缺失子节...]), ...]  —— --strict 模式下阻断定稿
      warns    = [(heading, [缺失子节...]), ...]  —— 非 strict 仅提醒
    豁免：已有-沿用（tag r）只写一句、跨章节引用小节（同 4.x/见 4.x）。
    """
    import re as _re
    ...
    for heading, body in blocks:
        ...
        if ref_pat.search(body):
            continue
        miss = []
        has_proto = ('<img' in body) or ('页面原型与交互示意' in body) or ('原型示意' in body)
        has_detail_section = ('详细逻辑规则' in body)
        has_show = ('界面与展示规则' in body) or ('入参校验' in body) or ('极限值处理' in body)
        has_boundary = ('业务逻辑与边界条件' in body) or ('未登录拦截' in body) or ('防刷控制' in body) or ('按钮状态切换' in body)
        if not has_proto:
            miss.append('2.1 页面原型与交互示意(<img>截图)')
        if not (has_detail_section or (has_show and has_boundary)):
            miss.append('2.2 详细逻辑规则(界面与展示规则 + 业务逻辑与边界条件)')
        if miss:
            hits.append((heading, miss))
    return (hits, [])
...
def main():
    ...
    print("# PRD 结构排查报告")
    print()
    print(f"文件：`{os.path.basename(path)}`" + ("  **[--strict 阻断模式]**" if strict else ""))
    print()
    ...
    # ---------- 阶段2.5：本期新增/改动页面结构（docx 每页模板 2.1~2.2）----------
    print()
    print("## 阶段2.5：本期新增/改动页面结构（docx 每页模板 · 2.1 原型截图 + 2.2 详细逻辑规则）")
    print()
    page_blockers, _ = scan_page_structure(raw_html if raw_html else text)
    if not page_blockers:
        print("✅ 所有本期新增/改动页面均齐备「页面原型与交互示意(<img>) + 详细逻辑规则(界面与展示规则+业务逻辑与边界条件)」。")
    else:
        for heading, miss in page_blockers:
            tag = "🔴 **不通过(阻断)**" if strict else "⚠️ **缺失(建议补)**"
            print(f"- {tag} 功能点「{heading}」缺：{'; '.join(miss)}")
        if strict:
            print()
            print(f"⛔ **--strict 阻断**：以上 {len(page_blockers)} 个页面未达 docx 每页模板必备子节，"
                  "须补齐 2.1 原型截图 + 2.2 详细逻辑规则后方可定稿（不得仅写一段扁平描述）。")
    ...
    print()
    if strict and page_blockers:
        print(f"⛔ **--strict 不通过**：{len(page_blockers)} 个本期新增/改动页面缺 docx 每页模板必备子节"
              "（2.1 原型截图 / 2.2 详细逻辑规则）。须补齐后重新跑 `check_prd.py <prd> --strict` 通过方可定稿。")
        print()
    elif page_blockers:
        print(f"ℹ️ **每页结构提示**：{len(page_blockers)} 个页面缺 docx 每页模板子节（非 strict 不阻断，"
              "建议补齐以符合《PRD 输出规范 260715》）。")
        print()
    if depth_missing:
