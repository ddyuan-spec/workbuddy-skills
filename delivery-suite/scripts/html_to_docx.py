...
def add_inline(paragraph, node, bold=False):
    ...
    if tag in ("b","strong"):
        ...
        add_img(paragraph, node)
    elif tag == "svg":
        add_svg(paragraph, node)
    else:
        for ch in node.children: add_inline(paragraph, ch, bold=bold)
...
def add_img(parent, el, width=5.8):
    ...
    inline = isinstance(parent, Paragraph)
    if not src or src.startswith("http"):
        if alt:
            if inline:
                r = parent.add_run("[缺失图片] "+alt); r.font.color.rgb=RGBColor(0xdc,0x26,0x26); _cjk(r)
            else:
    ...
    if inline:
        # 内联上下文（图片直接包在 <p> 内）：把图片作为 run 嵌入当前段落
        parent.add_run().add_picture(io.BytesIO(data), width=Inches(width))
        if alt:
            r = parent.add_run("  "+alt); r.italic = True; r.font.size = Pt(9); r.font.color.rgb = RGBColor(0x6b,0x72,0x80); _cjk(r)
    else:
...
def add_svg(parent, el, width=6.2):
    ...
    if png and os.path.exists(png):
        ...
        msg = "[流程图渲染失败]"
        if inline:
            r = parent.add_run(msg); r.font.color.rgb=RGBColor(0xdc,0x26,0x26); _cjk(r)
        else:
            p = parent.add_paragraph(); r = p.add_run(msg); r.font.color.rgb=RGBColor(0xdc,0x26,0x26); _cjk(r)
